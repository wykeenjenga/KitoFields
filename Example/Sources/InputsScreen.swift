...
// MARK: - Sign-up form, but validated with KitoFormController

/// Same fields as `SignUpFormDemo`, but instead of tracking one `@State var xValid` per field and
/// disabling the button, a single `KitoFormController` validates every field on submit and moves
/// the keyboard straight to the first one that failed.
